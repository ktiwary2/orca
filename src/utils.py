...
def imread_exr_npy(filename, **params):
    arr = np.load(filename, allow_pickle=True).astype(np.float32)
    return arr
...
def visualize_depth(depth, to_tensor=True, cmap=cv2.COLORMAP_TURBO):
    """
    depth: (H, W)
    --> Magma, "Viridus" 
    """
    x = depth.numpy()
    _NEAR_DEPTH_CLIP = 0.0
    # _FAR_DEPTH_CLIP = 6.0
    _FAR_DEPTH_CLIP = 4.0
    x = np.clip(x, _NEAR_DEPTH_CLIP, _FAR_DEPTH_CLIP) # clip it
