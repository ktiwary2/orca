...
class ImplicitSurface(nn.Module):
    ...
    def forward_with_nablas_curvature(self,  x: torch.Tensor, has_grad_bypass: bool = None):
        has_grad = torch.is_grad_enabled() if has_grad_bypass is None else has_grad_bypass
        has_grad = True # have to take gradients wrt nablas regardless of if torch.grad() 
        ...
        with torch.enable_grad():
            ...
            nabla = autograd.grad(
                ...
                torch.ones_like(implicit_surface_val, device=x.device),
                create_graph=has_grad, 
                allow_unused=has_grad,
                retain_graph=has_grad,
                only_inputs=has_grad)[0]

            nabla.requires_grad_(True)

            nabla_norm = torch.nn.functional.normalize(nabla, p=2.0, 
                                                       dim=-1)
            curvature_x = autograd.grad(
                nabla_norm[..., 0],
                x,
                torch.ones_like(nabla[..., 0], device=x.device),
                create_graph=has_grad,
                allow_unused=has_grad,
                retain_graph=has_grad,
                only_inputs=True)[0]

            curvature_y = autograd.grad(
                nabla_norm[..., 1],
                x,
                torch.ones_like(nabla[..., 1], device=x.device),
                create_graph=has_grad,
                allow_unused=has_grad,
                retain_graph=has_grad,
                only_inputs=True)[0]

            curvature_z = autograd.grad(
                nabla_norm[..., 2],
                x,
                torch.ones_like(nabla[..., 2], device=x.device),
                create_graph=has_grad,
                allow_unused=has_grad,
                retain_graph=has_grad,
                only_inputs=True)[0]

            curvature = curvature_x[...,0] + curvature_y[...,1] + curvature_z[...,2]
        ...
        if not has_grad:
            ...
            h = h.detach()
        return implicit_surface_val, nabla, curvature, h
...
class IntFresEnvMapNet(nn.Module):
    def __init__(self,
        embedding_fn=None,
        D=4,
        W=256,
    ...
    def forward(
        ...
        cos_theta_d:torch.Tensor):

        if self.sh_embed:
            radiance_input = torch.cat([self.embed_fn_view(dirs=view_dirs, roughness=roughness[...,0], L=self.L),
                                        cos_theta_d],dim=-1)
        else:
            radiance_input = torch.cat([self.embed_fn_view(view_dirs), 
                                        roughness,
                                        cos_theta_d], dim=-1)
...
class IntEnvMapNet(nn.Module):
    ...
    def forward(
        ...
        print("inside spec.forward--> ", view_dirs.shape, roughness.shape)
        if self.sh_embed:
        ...
        print("inside radiance_input--> ", radiance_input.shape)
        h = radiance_input
        for i in range(self.D+1):
            if i in self.skips:
                h = torch.cat([h, radiance_input], dim=-1)
...
class Expmin1(nn.Module):
    def __init__(self, beta=1.0):
        super(Expmin1, self).__init__()
        self.beta = beta
    def forward(self,x):
        return self.beta*torch.exp(x) - 1
        ...
        for l in range(D + 1):
            ...
            if l != D:
                ...
                if final_act == 'sigmoid':
                    ...
                    layer = DenseLayer(in_dim, out_dim, activation=nn.Identity())
                elif final_act == 'expmin1':
                    layer = DenseLayer(in_dim, out_dim, activation=Expmin1())
...
class SpecNet(nn.Module):
    ...
    def forward(
        ...
        view_dirs: torch.Tensor):
        # radiance_input = view_dirs
        radiance_input = self.embed_fn_view(view_dirs)
