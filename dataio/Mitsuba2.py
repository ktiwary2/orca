...
# bound_specular = lambda x, max_: torch.clip(linear_rgb_to_srgb(x), 0., max_)
# bound_specular = lambda x, max_: linear_rgb_to_srgb(torch.clip(x, 0., max_))
bound_specular = lambda x, max_: torch.clip(x, 0., max_)

try:
    import OpenEXR
    _FLAG_USE_NPY_AS_INPUT_ = False
except ImportError:
    _FLAG_USE_NPY_AS_INPUT_ = True
    print("Couldn't import OpenEXR, defaulting to npy files")
class SceneDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 sel_idx_offset=None,
                 sel_idx_sample=None, 
                 max_clip_specular=None # clip_specular to a max_value
                 ):
        ...
        self.max_clip_specular = max_clip_specular

        if self.max_clip_specular is not None: 
            print("-------[INFO] Clipping Diffuse + Specular ground truth to {}..".format(self.max_clip_specular))
        else:
            print("-------[INFO] NOT Clipping Diffuse + Specular..")


        if _FLAG_USE_NPY_AS_INPUT_:
            self._ext = ['*.png', '*.jpg', '*.JPEG', '*.JPG', '*.npy', '*.raw']
        else:
            self._ext = None

        image_dir = '{0}/image/'.format(self.instance_dir)
        image_paths = sorted(glob_imgs(image_dir, self._ext))
        mask_dir = '{0}/mask//'.format(self.instance_dir)
        mask_paths = sorted(glob_imgs(mask_dir, self._ext))
        if self.load_normals:
            normal_dir = '{0}/normal/'.format(self.instance_dir)
            normal_paths = sorted(glob_imgs(normal_dir, self._ext))
        ...
        if os.path.exists(specular_dir):
            ...
            specular_paths = sorted(glob_imgs(specular_dir, self._ext))
        else:
        ...
        # sel_idx = range(5) # for debugging only 
        # print('ONLY SELECTING 5 IAMGESS!!! CHANGE ME...')
        ...
        for path in tqdm(image_paths, desc='loading images...'):
            ...
            if self.max_clip_specular is not None: 
                rgb = bound_specular(rgb, self.max_clip_specular)
        ...
        if not self.only_diffuse:
            ...
            for path in specular_paths:
                ...
                spec_torch = torch.from_numpy(specular).float()
                if self.max_clip_specular is not None: 
                    spec_torch = bound_specular(spec_torch, self.max_clip_specular)
                self.specular_images.append(spec_torch)
...
if __name__ == "__main__":
    ...
    # dataset = SceneDataset(False, '../mitsuba2_render_multiview_v2/data/mitsuba2_renders/pokemon_v01_forward_origin_shift',
    #                        scale_radius=3)
    dataset = SceneDataset(False, '../mitsuba2_render_multiview_v2/data/mitsuba2_renders/living-room_v09_forward',
                           scale_radius=3)
    ...
    visualize(camera_matrix, extrinsics,label='v09')
