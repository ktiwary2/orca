...
try: 
    import OpenEXR
    from src.utils import imread_exr_ch as imread_exr
except ImportError as e: 
    print("Cannot import OpenEXR, using npy files instead...")
    from src.utils import imread_exr_npy as imread_exr
    

def glob_imgs(path, force_ext: List = None):
    imgs = []
    if force_ext is None: 
        for ext in ['*.png', '*.jpg', '*.JPEG', '*.JPG','*.exr','*.raw']:
            imgs.extend(glob.glob(os.path.join(path, ext)))
    else:
        for ext in force_ext:
            imgs.extend(glob.glob(os.path.join(path, ext)))
...
def load_rgb_exr(path, downscale=1):
    img = np.maximum(imread_exr(path, keys_list=[['R','G','B']]),0.)
    img = skimage.img_as_float32(img)
    if downscale != 1:
        img = rescale(img, 1./downscale, anti_aliasing=False, multichannel=True)
...
def load_normal_exr(path, downscale=1):
    img = np.clip(imread_exr(path, keys_list=[['R', 'G','B']]),-1.,1)
    img = skimage.img_as_float32(img)
    if downscale != 1:
        img = rescale(img, 1./downscale, anti_aliasing=False, multichannel=True)
...
def load_mask(path, downscale=1):
    ...
    return object_mask
def load_mask_from_rembg(path, downscale=1):
    import cv2
    img = imageio.imread(path)
    alpha = img[...,3]
    alpha = skimage.img_as_float32(alpha)
    ...
    # object_mask = alpha > 0.0
    # kernel = np.ones((7,7),np.uint8)
    # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7))
    # object_mask_di = cv2.dilate(object_mask.astype('uint8'),kernel,iterations=1)
    # object_mask = cv2.erode(object_mask_di,kernel,iterations=3)>0
    # object_mask = object_mask_di>0
    return object_mask
...
def backup(backup_dir):
    ...
    subdirs_to_copy = ["", "dataio/", "models/", "tools/", "debug_tools/", "utils/","src/","models/frameworks/"]
