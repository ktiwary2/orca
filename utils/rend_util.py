...
def create_intrinsic_matrix(hfov, H, W):
    """
    hfov must be in Degrees! 
    FOV = 2 arctan (x/2f)
    """
    # assert H == W, "Currently only rendering with hfov..."
    hfov_rad = hfov * (np.pi/180.)
    fx = (W/2.)/ np.tan(hfov_rad/2.)
    fy = (H/2.)/ np.tan(hfov_rad/2.)
    # fy = (H/2) / torch.tan(hfov_rad) # asssume the hfov == vfov
    K = np.array([[fx, 0., (W/2.)],
                  [0., fy, (H/2.)],
                  [0.,  0.,  1. ]])
    return torch.tensor(K).float()
...
def get_rays(c2w, intrinsics, H, W, N_rays=-1, compute_radii=False):
    device = c2w.device
    ...
    # 1) First compute c2w on all rays for a given pose 
    ...
    # compute_radii
    if compute_radii:
        norm_dirs = rays_d.reshape(1, H, W, 3)
        # norm_dirs = F.normalize(rays_d, dim=2).reshape(1, H, W, 3)
            # Distance from each unit-norm direction vector to its x-axis neighbor.
        dx = torch.sqrt(torch.sum((norm_dirs[:, :-1, :, :] - norm_dirs[:, 1:, :, :])**2, -1))
        dx_norm = torch.concat([dx, dx[:, -2:-1, :]], 1)
        # dy = torch.sum((norm_dirs[:, :, :-1, :] - norm_dirs[:, :, 1:, :])**2, -1)
        # dy_norm = torch.concat([dy, dy[:, :, -2:-1]], 2)
        # Cut the distance in half, and then round it out so that it's
        # halfway between inscribed by / circumscribed about the pixel.
        radii = dx_norm[..., None] * 2 / torch.sqrt(torch.tensor(12))
        # radii = (0.5 * (dx_norm + dy_norm))[..., None] * 2 / torch.sqrt(torch.tensor(12))
        radii = radii.reshape(rays_d.unsqueeze(2).shape[:3])

    rays_o = cam_loc[..., None, :].expand_as(rays_d)

    # 2) after all computation just select args.N_rays from all rays
    # import pdb 
    # pdb.set_trace()
    # N_rays = 64
    if N_rays > 0:
        ...
        select_inds = torch.randint(0, H*W, size=[N_rays]).to(device)
        ...
        if compute_radii:
            ...
            rays_d = torch.concat([rays_d, radii], dim=-1)
        select_inds = select_inds.expand([*prefix, N_rays])
    else:
        if compute_radii:
            rays_d = torch.concat([rays_d, radii], dim=-1)
